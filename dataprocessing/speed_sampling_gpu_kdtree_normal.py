...
def point_rand_sample_bound_points(numsamples, dim, 
    ...
    with torch.no_grad():
        ...
        # normals at start points
        n0_mean_t, n0_var_t = mc_normal_stats(
            X[:, :dim],
            v_obs,
            n_obs,
            kdtree,
        ...
        # normals at goal points
        n1_mean_t, n1_var_t = mc_normal_stats(
            X[:, dim:dim+dim],
            v_obs,
            n_obs,
            kdtree,
    ...
    # concatenate into final layout
    normal_mean = torch.cat((n0_mean_t, n1_mean_t), dim=1).detach().cpu().numpy()
    normal_var  = torch.cat(
        (n0_var_t.unsqueeze(1), n1_var_t.unsqueeze(1)),
        dim=1
    ).detach().cpu().numpy()
    # ----------------------------
    # ---- deterministic case ----
